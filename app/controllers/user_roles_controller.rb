class UserRolesController < ApplicationController
  ...
  def create
    user_already_present = UserRole.where(user_role_params)
    @user_role = UserRole.new(user_role_params)
    respond_to do |format|
      if !user_already_present.present?
        if @user_role.save
        ...
        format.html { redirect_to user_roles_path, notice: 'Record already present.' }
        format.json { render :show, status: :created, location: @user_role }
      end
    end
  end
  ...
  def update
    respond_to do |format|
      if @user_role.update(user_role_params)
        format.html { redirect_to user_roles_path, notice: 'User role was successfully updated.' }
